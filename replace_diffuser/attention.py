...
device2 = torch.device('cuda:0') if torch.cuda.is_available() else torch.device('cpu')
device3 = torch.device('cuda:0') if torch.cuda.is_available() else torch.device('cpu')
'''
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, hidden_states, context=None, timestep=None):
        ...
        new_hidden_states, cross_attn_prob = self.attn1(norm_hidden_states)
        hidden_states = new_hidden_states + hidden_states
        ...
        #print("norm_hidden_states cross: ",norm_hidden_states.shape)
        new_hidden_states, cross_attn_prob = self.attn2(norm_hidden_states, context=context)
        hidden_states = new_hidden_states + hidden_states
